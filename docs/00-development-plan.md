...
대전에서 여행 중인 3~6명이 기존 일정에 문제가 생겼을 때 다음 흐름을 반응형 웹에서 완료할 수 있는 1차 공개 베타를 배포한다. React Native 앱은 웹 MVP 안정화 후 같은 API·권한·상태 계약으로 확장한다.
...
1. 화면별로 따로 만들지 않고 `DB → API → 웹 → 테스트`가 연결되는 작은 세로 기능 단위로 먼저 완성한다.
2. 모바일은 웹 MVP 이후 같은 API·상태·권한 규칙을 재사용해 구현하되 UI 컴포넌트 자체는 공유하지 않는다.
3. 모바일 기능 개발을 미루는 동안 기존 Expo 기반의 lint·typecheck·export 회귀 검사는 유지한다.
4. 공정성, 투표, 날씨 임계값은 결정론 코드로 구현한다. LLM 결과를 직접 저장하거나 확정하지 않는다.
5. 개인 선호·예산·피로도·투표 원본은 그룹 API, 로그, 분석 이벤트, Bedrock 프롬프트에 넣지 않는다.
6. 외부 데이터가 없으면 추정하지 않고 `정보 없음`을 반환한다.
7. 각 단계는 테스트와 staging 확인이 끝나야 다음 단계로 넘어간다.
8. Production에 수동으로 만든 AWS 리소스는 허용하지 않고 CDK에 기록한다.
...
| 1. 개발 기반 | 1~2주 | 웹·API·DB·CI 연결과 기존 Expo 골격 회귀 검사 |
...
| 7. 웹 제품화 | 12~13주 | UI 시안 반영, 인앱 알림, 웹 접근성 |
| 8. 웹 공개 베타 | 14~16주 | AWS 운영, 보안·성능·복구 검증 |
| 9. 모바일 제품화 | 웹 베타 후 별도 산정 | 웹 MVP 기능 이식, 푸시·딥링크·스토어 출시 |
| 10. Docker 전환 | 베타 후 2~3주 | Beanstalk에서 ECS Fargate로 이전 |
...
- Expo/EAS 계정, App Store Connect, Google Play Console은 웹 베타 안정화 후 준비를 재개
...
- 웹이 API base URL 설정으로 health 응답을 표시하게 하고 기존 Expo 골격의 빌드 회귀만 확인한다.
...
> 로컬에서 PostgreSQL·Spring API·Next.js가 실행되고 웹이 API health 상태를 표시한다.
...
- Cognito User Pool, 웹 client, callback URL과 PKCE 흐름을 우선 구성한다. 앱 client는 모바일 제품화 단계에서 추가한다.
...
- 온보딩, 로그인, 여행 생성, 여행 목록, 초대·가입 화면을 웹에 구현한다.

두 번째 제품 마일스톤:

> 로그인한 방장이 여행을 만들고 세 명의 사용자가 초대 링크로 참여한 뒤 웹에서 같은 여행을 확인한다.

완료 조건:

- 인증되지 않은 요청은 `401`, 권한 없는 멤버는 `403`을 받는다.
- 만료 초대는 `410`, 인원 초과는 `TRIP_FULL` 오류를 받는다.
- 웹 초대 링크가 로그인 후 원래 가입 흐름으로 복귀한다.
- 사용자가 웹에서 계정 삭제를 요청할 수 있다.
...
- 생성 진행, 후보 카드, 실패·후보 부족 화면을 웹에 만든다.
...
- `ui/`의 9개 시안을 실제 제품 범위에 맞게 반응형 웹에 반영한다.
...
- 웹 WCAG 2.2 AA와 키보드 접근성을 검증한다.
- 한국어 문구와 개인정보·AI 설명을 최종 검수한다.

완료 조건:

- 핵심 기능이 지원 웹 브라우저와 모바일 화면 크기에서 동일하게 동작한다.
...
- Playwright, Testcontainers, WireMock, 부하·보안 테스트를 통과한다.
- 개인정보처리방침과 이용약관을 준비한다.

출시 게이트:

- 지원 웹 브라우저 대표 E2E 전체 통과
...
1. FND-05 실기기 검증과 FND-12 앱 health 세로 슬라이스를 완료한다.
2. 웹 MVP의 로그인, 여행·초대, 일정, 재조정, 투표·확정 흐름을 Expo 앱으로 확장한다.
3. 공용 OpenAPI 생성 클라이언트와 권한·오류·상태 계약을 그대로 사용한다.
4. Expo Push, 초대 딥링크, SecureStore 기반 인증을 구현한다.
5. VoiceOver·TalkBack, 네트워크 중단·재연결, iOS·Android Maestro E2E를 검증한다.
6. 개인정보 라벨과 심사 계정을 준비하고 EAS Build/Submit으로 스토어 출시를 진행한다.

완료 조건:

- 핵심 기능이 웹·iOS·Android에서 같은 도메인 규칙으로 동작한다.
- 모바일 전용 구현 때문에 API 계약이나 웹 동작이 분기되지 않는다.
- 실제 기기에서 로그인 redirect, 초대 딥링크, 푸시, 오프라인 복구가 통과한다.

### 단계 10. Docker와 ECS 전환
...
| 계약 | Spring OpenAPI, 생성 TS client | 웹 DTO 호환성과 후속 모바일이 재사용할 플랫폼 중립 계약 |
| 웹 E2E | Playwright | 로그인부터 일정 확정까지 전체 흐름 |
| 앱 E2E | Maestro | 웹 베타 후 iOS·Android 로그인, 딥링크, 푸시, 투표 |
...
- 웹의 정상·로딩·빈 상태·오류 상태가 구현됐다.
- 단위·통합 테스트와 필요한 E2E가 통과한다.
- 로그에 토큰·개인 선호·투표·프롬프트가 남지 않는다.
- staging에서 지원 브라우저로 확인했다.
- 모바일 제품화 범위에서는 같은 완료 정의에 실제 기기와 모바일 접근성 검증을 추가한다.
...
- [ ] AWS·Kakao·Google·Apple 개발 계정 상태 확인
- [ ] TourAPI `KorService2`, 기상청, ODsay 키 신청
- [ ] Bedrock 모델 접근 확인
- [ ] 모노레포 디렉터리와 root toolchain 생성
- [ ] Next.js 15 웹 생성
- [x] Expo 앱 골격 생성, 신규 모바일 기능은 웹 MVP 이후로 연기
- [ ] Java 21·Spring Boot 4.1 API 생성
- [x] 로컬 PostgreSQL과 최초 Flyway migration 생성
- [x] 웹에서 생성 client 기반 `/api/v1/system/health` 확인
- [ ] GitHub Actions 기본 build·test 구성

첫 번째 제품 세로 슬라이스는 `로그인 → 여행 생성 → DB 저장 → 웹 조회`를 완성한다. 모바일 화면은 웹 MVP 안정화 후 같은 계약으로 확장한다.
