...
| TourAPI `KorService2` | 대전 장소 검색 | `MOCK_READY` | 일일 할당량·이용조건 확인 | 활용 신청 후 실제 장소·운영 정보 어댑터를 연결하고 예제 응답 보존 |
| 기상청 단기예보 | 강수확률 자동 감지 | `MOCK_READY` | 일일 할당량·발표 주기 확인 | 활용 신청 후 실제 어댑터·스케줄을 연결하고 예제 응답 보존 |
| ODsay Basic | 대중교통 이동시간 | `MOCK_READY` | 일일 한도·계약 시작일로부터 6개월 종료 확인 | 신청 후 종료 예정일을 기록하고 실제 이동시간 어댑터 연결 |
