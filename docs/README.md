...
12. [AWS 계정 준비](./11-aws-account-readiness.md) - MFA·SSO·비용 알림·권한 분리 검증 절차
...
| 플랫폼 | 1차 공개 베타는 Next.js 웹으로 제공하고 React Native 앱은 웹 MVP 이후 동일한 핵심 기능으로 확장 |
...
- 기능은 `DB → API → 웹 → 테스트` 순서로 먼저 완성하고 모바일은 웹 MVP 이후 공용 OpenAPI 계약을 재사용해 구현한다.
- 모바일 기능 개발을 미루는 동안에도 기존 Expo 기반의 lint·typecheck·export 회귀 검사는 유지한다.
- API나 상태 전이를 바꾸면 공용 계약과 웹 테스트를 함께 바꾸고, 모바일 재개 시 같은 규칙을 적용한다.
